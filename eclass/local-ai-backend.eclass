...
# @FUNCTION: local-ai-backend_gen_run_sh
# @USAGE: [-e VAR=value]... <exec-name> [VAR=rel-path]...
# @DESCRIPTION:
# Generate ${T}/run.sh — the entry point the LocalAI server invokes to
# start this backend; local-ai-backend_install_meta installs it in
# preference to a FILESDIR copy. Each -e VAR=value is exported
# verbatim; each trailing VAR=rel-path argument becomes an exported
# VAR="${CURDIR}/rel-path"; LD_LIBRARY_PATH is appended to rather than
# replaced, so a caller's environment survives.
local-ai-backend_gen_run_sh() {
	local -a rawenv=()
	while [[ $1 == -e ]]; do
		rawenv+=( "$2" ); shift 2
	done
	local exe=$1; shift
	{
		printf '%s\n' '#!/bin/sh' \
			...
			"# Generated by local-ai-backend.eclass for ${CATEGORY}/${PN}." \
		...
		for kv in "$@"; do
			var=${kv%%=*} val=${kv#*=}
			if [[ ${var} == LD_LIBRARY_PATH ]]; then
				printf 'LD_LIBRARY_PATH="${CURDIR}/%s${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}"\n' "${val}"
			else
				printf '%s="${CURDIR}/%s"\n' "${var}" "${val}"
			fi
			printf 'export %s\n' "${var}"
		done
		printf 'exec "${CURDIR}/%s" "$@"\n' "${exe}"
	} > "${T}"/run.sh || die
}

# @FUNCTION: local-ai-backend_install_meta
# @USAGE: <backend-name> [alias]
# @DESCRIPTION:
# Install the pieces every backend directory needs for discovery: run.sh
# (generated by local-ai-backend_gen_run_sh, or a FILESDIR copy for a
# launcher too odd to generate) and a metadata.json naming the backend.
# With [alias], the variant registers as an alias candidate: the server
# resolves the alias to the best installed variant by host capability,
# while the concrete <backend-name> stays individually addressable.
local-ai-backend_install_meta() {
	...
	if [[ -n ${alias} ]]; then
		printf '{\n\t"name": "%s",\n\t"alias": "%s"\n}\n' \
			"${name}" "${alias}" > "${T}"/metadata.json || die
	else
...
local-ai-backend_install() {
	...
	local-ai-backend_install_meta "${name}" "${alias}"
}
